...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── REEL: Step 1 — Init via /{page_id}/video_reels ──────
    if (action === "reel_init") {
      const params = enc({ upload_phase:"START", access_token:token });
      const res = await new Promise((resolve, reject) => {
        const buf = Buffer.from(params, "utf8");
        const opts = {
          hostname: "graph.facebook.com",
          path: `/v19.0/${pageId}/video_reels`,
          method: "POST",
    ...
    // ── REEL: Step 2 — Upload chunk to upload_url ─────────
    if (action === "reel_chunk") {
      const { chunkBase64, startOffset, totalSize, uploadSessionId, uploadUrl } = body;
      ...
      const targetUrl = uploadUrl || `https://rupload.facebook.com/video-upload/v19.0/${uploadSessionId}`;
      const res = await fbVideoChunk(targetUrl, chunkBuf, startOffset, totalSize, uploadSessionId, token);
      return { statusCode:200, headers:CORS, body:JSON.stringify(res) };
    }

    // ── REEL: Step 3 — Finish via /{page_id}/video_reels ──
    if (action === "reel_finish") {
      const { uploadSessionId, description, scheduledTime } = body;
      const isScheduled = scheduledTime !== null && scheduledTime !== undefined && scheduledTime !== "null" && Number(scheduledTime) > 0;

      // FINISH phase — ใช้ SCHEDULED ถ้าตั้งเวลา, PUBLISHED ถ้าโพสต์เดี๋ยวนี้
      // video_reels รองรับ DRAFT | PUBLISHED | SCHEDULED
      const params = {
        upload_phase: "FINISH",
      ...
      // ถ้า PUBLISHED error → ลอง DRAFT แล้ว publish แยก
      if (res.error && !isScheduled) {
        console.log("PUBLISHED failed, trying DRAFT+publish:", res.error.message);
        const draftParams = enc({ upload_phase:"FINISH", video_id:uploadSessionId, video_state:"DRAFT", description:description||"", access_token:token });
        const draftRes = await new Promise((resolve, reject) => {
          const buf = Buffer.from(draftParams, "utf8");
          const opts = { hostname:"graph.facebook.com", path:`/v19.0/${pageId}/video_reels`, method:"POST", headers:{"Content-Type":"application/x-www-form-urlencoded","Content-Length":buf.length} };
          const req = https.request(opts, r => { let d=""; r.on("data",c=>d+=c); r.on("end",()=>{ try{resolve(JSON.parse(d));}catch{resolve({raw:d});} }); });
          req.on("error", reject); req.write(buf); req.end();
        });
        return { statusCode:200, headers:CORS, body:JSON.stringify(draftRes) };
      }
